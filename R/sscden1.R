                    ...
                    prec=1e-7,maxiter=30,skip.iter=FALSE)
{
    ...
    if (!yfac) {
        rho <- ssden(response,id.basis=id.basis,data=data,weights=cnt,
                     alpha=2,domain=ydomain,quad=yquad)
        rho.pt <- rho$quad$pt
        rho.wt <- dssden(rho,rho.pt)*rho$quad$wt
    }
    else {
        rho.pt <- data.frame(levels(mf[,ynames[1]]))
        if (length(ynames)>1) {
            for (ylab in ynames[-1]) {
                wk <- expand.grid(levels(mf[,ylab]),1:dim(rho.pt)[1])
                rho.pt <- data.frame(rho.pt[wk[,2],],wk[,1])
            }
        }
        colnames(rho.pt) <- ynames
        rho.wt <- as.vector(table(mf[,rev(ynames)]))
        rho.wt <- rho.wt/sum(rho.wt)
        rho <- list(pt=rho.pt,wt=rho.wt)
    }
        ...
        if (nphi) {
            ...
            for (i in 1:nphi) {
                ...
                if (is.null(xx)) {
                    ...
                    int.s <- c(int.s,sum(qd.s.wk*rho.wt))
                }
                else {
                    if (length(y.list)==0) {
                        names(xx) <- x.list
                        ...
                        for (j in 1:nx) {
                            ...
                            int.s.wk <- int.s.wk + sum(qd.s.wk*rho.wt)*xx.wt[j]
                        }
        ...
        if (nrk) {
            ...
            for (i in 1:nrk) {
                ...
                if (is.null(xx)) {
                    ...
                    int.r <- cbind(int.r,apply(rho.wt*qd.r.wk,2,sum))
                }
                else {
                    if (length(y.list)==0) {
                        names(xx) <- x.list
                        ...
                        for (j in 1:nx) {
                            ...
                            int.r.wk <- int.r.wk + apply(rho.wt*qd.r.wk,2,sum)*xx.wt[j]
                        }
